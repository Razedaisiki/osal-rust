...
fn callback_stops_self() {
    ...
    t.start().unwrap();
    let t2 = t.clone();
    let _t3 = MockTimer::new("self_stop", Duration::from_millis(100), TimerMode::OneShot,
...
fn callback_stops_another_timer() {
    ...
    let ta = MockTimer::new("A", Duration::from_millis(100), TimerMode::OneShot,
        Box::new(move || { af.store(true, Ordering::SeqCst); })).unwrap();
    let tb = MockTimer::new("B", Duration::from_millis(100), TimerMode::OneShot,
        Box::new(move || { bf.store(true, Ordering::SeqCst); })).unwrap();
    let tb2 = tb.clone();
    ...
    assert!(!b_fired.load(Ordering::SeqCst), "B should NOT fire (stopped by stopper)");
}
...
fn oneshot_re_trigger() {
    ...
    let t = MockTimer::new("t", Duration::from_millis(100), TimerMode::OneShot,
        Box::new(move || { f.fetch_add(1, Ordering::Relaxed); })).unwrap();
    t.start().unwrap();
...
fn periodic_not_reentrant() {
    ...
    let t = MockTimer::new("t", Duration::from_millis(100), TimerMode::Periodic,
        Box::new(move || { c.fetch_add(1, Ordering::Relaxed); })).unwrap();
    t.start().unwrap();
...
fn callback_in_flight_last_handle_dropped() {
    ...
    let f = Arc::clone(&fired);
    let t = MockTimer::new("t", Duration::from_millis(100), TimerMode::OneShot,
        Box::new(move || { f.store(true, Ordering::SeqCst); })).unwrap();
    let _t2 = t.clone();
    t.start().unwrap();
    MockClockControl.advance_clock(Duration::from_millis(200));
    assert!(fired.load(Ordering::SeqCst));
